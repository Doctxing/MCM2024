...
class Heredity:
    ...
    def select(self,pop):  # 使用随机抽样模拟自然选择
    ...
    def generate(self):
        ...
        for i in range(self.Iterations):
            print("迭代进度：",i,"/",self.Iterations,end="\r")
            pop = np.array(self.crossmuta(pop))
            pop = self.select(pop)
            fitness = self.getfitness(pop)
            temp = [np.argmax(fitness)]
            x, y = self.decodeDNA(pop[temp])
            if solution < self.function(x, y):
                solution_x = x
                solution_y = y
                solution   = self.function(solution_x,solution_y)  
        print("迭代进度：",self.Iterations,"/",self.Iterations,"...done")
...
if __name__ == '__main__':
    ...
    def restrict(x, y):  ##限制表达式，返回值为布尔值
        if x<2:
            ...
            return True
    Train = Heredity(bound_x_arr=[0,5], bound_y_arr=[0,5],
                     function=demoFun,   isMax=True,
                     restrict=restrict)
    ##此代码可用于调节种群·神職设置(默认值)
    # Train.Iterations = 100
    solution_x, solution_y = Train.generate()
    print(solution_x, solution_y,demoFun(solution_x,solution_y))